...
public class March_22 {
    ...
    public void lastNameTest(){
        ...
        BrowserUtils.wait(3);
        List<WebElement> lastNamesElements = driver.findElements(By.xpath("//table[1]//tbody//td[1]"));
        List<String> lastNames = new ArrayList<>();
        for (WebElement lastNamesElement : lastNamesElements) {
            lastNames.add(lastNamesElement.getText());
        }
        System.out.println("lastNames = " + lastNames);
        List<String> expected = new ArrayList<>(lastNames);
        Collections.sort(expected);
        System.out.println("expected = " + expected);
        Assert.assertEquals(lastNames,expected);
    }
    /**
